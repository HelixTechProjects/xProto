...
impl<'a> Lexer<'a> {
    ...
    pub fn new_want_comment(input: &'a str) -> Lexer<'a> {
        Lexer {
            input,
            pos: 0,
    ...
    // 截取注解的一行内容
    pub fn next_annotation_lit_raw(&mut self) -> LexerResult<String> {
        self.skip_whitespaces();
        let start = self.pos;
        while self.lookahead_char() != Some('\n') {
            self.next_byte_value()?;
        }
        let mut raw = String::new();
        self.next_char_expect_eq('\n')?;
        raw.push_str(&self.input[start..self.pos - 1]);
        Ok(raw)
    }

    pub fn next_comment_lit_raw(&mut self) -> LexerResult<String> {
        let start = self.pos;
        while self.lookahead_char() != Some('\n') {
            self.next_byte_value()?;
        }
    ...
    pub fn next_block_comment_lit_raw(&mut self) -> LexerResult<String> {
        let end = "*/";
        match self.rem_chars().find(end) {
            None => Err(LexerError::UnexpectedEof),
            Some(len) => {
                let new_pos = self.pos + len + end.len();
                let raw = self.input[self.pos..new_pos - end.len()].to_string();
                self.skip_to_pos(new_pos);
                Ok(raw)
            }
        }
    ...
    fn next_version_opt(&mut self) -> LexerResult<Option<String>> {
        if self.next_char_if_eq('-') {
            let major = self.next_decimal_digit()?;
            self.next_char_expect_eq('.')?;
            let minor = self.next_decimal_digit()?;
            self.next_char_expect_eq('.')?;
            let patch = self.next_decimal_digit()?;
    ...
    fn next_annotation_opt(&mut self) -> bool {
        self.skip_if_lookahead_is_str("//@")
    }

    fn next_comment_opt(&mut self) -> bool {
        self.skip_if_lookahead_is_str("//")
    }

    fn next_block_comment_opt(&mut self) -> bool {
        self.skip_if_lookahead_is_str("/*")
    }
    ...
    fn next_token_inner(&mut self) -> LexerResult<Token> {
        ...
        if let Some(version) = self.next_version_opt()? {
            return Ok(Token::Version(version));
        }

        if self.next_annotation_opt() {
            let lit = self.next_annotation_lit_raw()?;
            return Ok(Token::Annotation(lit));
        }
        // 需要comment
        if !self.is_skip_comment {
            //
            if self.next_comment_opt() {
                let lit = self.next_comment_lit_raw()?;

                return Ok(Token::Comment(lit));
            }
            if self.next_block_comment_opt() {
                let lit = self.next_block_comment_lit_raw()?;
                return Ok(Token::BlockComment(lit));
            }
        }
